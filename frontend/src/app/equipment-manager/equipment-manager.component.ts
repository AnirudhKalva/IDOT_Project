...
//     if (!this.selectedCounty || !this.selectedQuarter || !this.fuelPriceData) {
//       return;
//     }
  
//     const gasRow = this.fuelPriceData.find(row =>
//       row.County === this.selectedCounty &&
//       row.Quarter === this.selectedQuarter &&
//       row['Fuel Type'] === 'Gas'
//     );
  
//     const dieselRow = this.fuelPriceData.find(row =>
//       row.County === this.selectedCounty &&
//       row.Quarter === this.selectedQuarter &&
//       row['Fuel Type'] === 'Diesel'
//     );
  
//     const otherRow = this.fuelPriceData.find(row =>
//       row.County === this.selectedCounty &&
//       row.Quarter === this.selectedQuarter &&
//       row['Fuel Type'] === 'Other'
//     );
  
//     this.fuelCostsForm?.patchValue({
//       gasoline_price: gasRow ? parseFloat(gasRow['Fuel Price']) : 0,
//       diesel_price: dieselRow ? parseFloat(dieselRow['Fuel Price']) : 0,
//       other: otherRow ? parseFloat(otherRow['Fuel Price']) : 0
//     });
//   }
...
//     this.fuelPriceService.getFuelPriceData().subscribe((data: FuelPriceRow[]) => {
//       this.fuelPriceData = data;
//       this.counties = [...new Set(data.map(entry => entry.County))].sort();
//       this.quarters = [...new Set(data.map(entry => entry.Quarter))].sort();
...
export class EquipmentManagerComponent implements OnInit {
  ...
  // }
    ...
    //  Updated Save Function (Handles Manual or CSV Upload)
    editFuelCosts() {
    ...
    handleCsvUpload(event: any) {
      ...
      Papa.parse(file, {
        ...
        complete: (result) => {
          ...
          console.log('Parsed CSV Data:', this.parsedCsvData);
        },
        error: (error) => {
    ...
    //  Upload Parsed CSV Data to Backend
    uploadCsvData() {
      if (this.parsedCsvData.length === 0) {
        this.notificationService.triggerNotification('No CSV data to upload.', 'error');
        return;
      }
  
      this.dataLoaded = false;
  
      this.fuelPriceService.bulkUploadFuelPrices(this.parsedCsvData).subscribe(
        () => {
          this.notificationService.triggerNotification('CSV data uploaded successfully!', 'success');
          this.closeFuelForm();
          this.dataLoaded = true;
        },
        (error) => {
          console.error('CSV upload error:', error);
          this.notificationService.triggerNotification('CSV upload failed.', 'error');
          this.dataLoaded = true;
        }
      );
    }
  ...
  generate2025Data(priceIncreaseRate: number, dataUpdate: boolean) {
    ...
    this.userService.generateNextYearEquipData(priceIncreaseRate, dataUpdate).subscribe(
      (response) => {
        const generatedYear = response?.year || 'Next'; 
        console.log(`Response from generate ${generatedYear}:`, response);
        this.notificationService.triggerNotification(
          `${generatedYear} data generated successfully`, 
          'success'
        );
